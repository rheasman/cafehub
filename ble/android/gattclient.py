...
from ble.android.pybluetoothgattcallback import AndroidConnState, AndroidGATTStatus, PyBluetoothGattCallback
from ble.bleexceptions import *
from ble.bleops import ContextConverter, CountingSemaphore, QOp, QOpExecutor, QOpManager, async_wrap_async_into_QOp, synchronized_with_lock, wrap_into_QOp
...
# TODO: THIS WILL PROBABLY BREAK FOR MORE THAN ONE GATT CLIENT
...
BLE_UUID = autoclass('java.util.UUID')
...
class GATTClient(GATTClientInterface):
    ...
    def __init__(self, macaddress: str, qopexecutor: QOpExecutor, contextconverter: ContextConverter):
        ...
        self.NotifyCallback : dict[str, Union[Callable, None]] = {}
    ...
    def _raise_if_disconnected(self):
        state, cstate = self._get_conn_info()
    ...
    def _set_notify(self, uuid : CHAR_UUID, enable : bool, notifycallback : Callable, manager: Union[QOpManager, None] = None, reason: Union[str, None] = None):
    ...
    def _procStateChange(self, manager = None, reason = None):
        """
    ...
    def _char_write(self, uuid: CHAR_UUID, data : bytes, manager: Union[QOpManager, None] = None, reason: Union[str, None] = None) -> None:
    ...
    def _desc_write(self, charuuid : CHAR_UUID, descid : DESC_UUID, data : bytes, manager: Union[QOpManager, None] = None, reason: Union[str, None] = None) -> None:
        ...
        # Don't do this check either, because we can't match callbacks with writes, as callbacks are inconsistently called
    ...
    async def async_char_write(self, uuid: CHAR_UUID, data):
        """
    ...
    def set_notify(self, uuid : CHAR_UUID, enable : bool, notifycallback : Callable):
        """
    ...
    def char_write(self, uuid: CHAR_UUID, data: bytes) -> None:
        """
        Synchronous write to device. Write occurs in a background thread,
        but the calling thread is made to wait until there is a result.
        """
        Logger.debug("BLE: char_write(%s)" % (uuid,))
    ...
    def callback_char_write(self, uuid: CHAR_UUID, data, callback=None):
        """
    ...
    def descriptorRead(self, uuid: CHAR_UUID, etc):
        pass
